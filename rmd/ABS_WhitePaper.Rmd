...
tier_summary <- df %>%
  ...
  mutate(
    tier = case_when(
      str_detect(call_result_tiered, "T1") ~ "< 0.1\" (Broadcast threshold)",
      str_detect(call_result_tiered, "T2") ~ "0.1\" to ¼ baseball",
      str_detect(call_result_tiered, "T3") ~ "¼ to ½ baseball",
      str_detect(call_result_tiered, "T4") ~ "½ to 1 baseball",
...
The majority of missed calls — **`r tier_summary %>% filter(str_detect(tier, "T1|T2")) %>% summarise(p = percent(sum(as.numeric(str_remove_all(pct,"%")))/100)) %>% pull(p)`** — fell within one quarter of a baseball width from the zone boundary. These are genuinely difficult calls that reasonable people can disagree on. But **`r tier_summary %>% filter(str_detect(tier, "T4|T5|T6")) %>% summarise(n = format(sum(as.numeric(str_remove_all(n,","))), big.mark=",")) %>% pull(n)`** missed calls were wrong by more than half a baseball width. Those are not close calls.
